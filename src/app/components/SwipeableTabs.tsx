...
export default function SwipeableTabButtons() {
  ...
  const fetchProducts = async (index: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:5187/api/ProductsController21/ByCategoryName/${category}?pageNumber=1&pageSize=4`);
  ...
  const fetchMoreProducts = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5187/api/ProductsController21/ByCategoryName/${category}?pageNumber=${nextPage}&pageSize=4`);
      const data: Product[] = await response.json();

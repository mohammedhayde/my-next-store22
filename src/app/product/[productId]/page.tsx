...
const fetchHtmlContent = async (url, setContent) => {
  try {
    // تحقق من أن الرابط ينتهي بملف .html أو .txt
    if (url.endsWith('.html') || url.endsWith('.txt')) {
      const response = await fetch(url);
      const data = await response.text();
      setContent(data);
    } else {
      console.error('URL must end with .html or .txt');
    }
  } catch (error) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (loadedProduct?.description) {
        fetchHtmlContent(loadedProduct.description, setContent);
      }
    }
